...
int distance(const struct snake snake,
             const struct food food) { // вычисляет количество ходов до еды
...
void autoChangeDirection(struct snake *snake, struct food food[],
  ...
  for (int i = 1; i < foodSize; i++) { // ищем ближайшую еду
    pointer = (distance(*snake, food[i]) < distance(*snake, food[pointer]))
                  ? i
                  : pointer;
  }
...
int checkDirection(int32_t dir, int32_t key) {
  if (KEY_DOWN == key && dir == UP || KEY_UP == key && dir == DOWN ||
      KEY_LEFT == key && dir == RIGHT || KEY_RIGHT == key && dir == LEFT) {
    return 0;
  } else {
    return 1;
  }
...
void init(char *playerName, struct snake *head, struct tail *tail,
          size_t size) {
  clear(); // очищаем весь экран
...
int localStrcmp(const char *strSource, const char *strTarget) {
  int tmp1 = 0;
  int tmp2 = 0;
  while (*strSource++)
    tmp2 += *strSource;
...
_Bool isPlayerImmortal(struct snake *head) {
  if (localStrcmp(head->playerName, "immortalSnake") == 0)
    return 1;
  return 0;
}
/*
...
void printHelp(char *s) { mvprintw(0, 0, s); }
...
void repairSeed(struct food f[], size_t nfood, struct snake *head) {
  ...
  for (size_t i = 0; i < nfood; i++)
    for (size_t j = 0; j < nfood; j++) {
      ...
      if (i != j && f[i].enable && f[j].enable && f[j].x == f[i].x &&
...
void printLevel(struct snake *head) {
  int max_x = 0, max_y = 0;
  getmaxyx(stdscr, max_y, max_x);
  if (isPlayerImmortal(head)) {
    mvprintw(0, max_x / 2, "IMMORTALSNAKE LEVEL: %lu", head->tsize);
  } else {
    mvprintw(0, max_x - 17, "%s LEVEL: %lu\t", head->playerName, head->tsize);
  }
}

void printExit(struct snake *head, int offset_x, int offset_y) {
  int max_x = 0, max_y = 0;
  getmaxyx(stdscr, max_y, max_x);
  mvprintw(max_y / 2 - offset_y, max_x / 2 - offset_x, "%s, your LEVEL is %d",
           head->playerName, head->tsize);
}
...
int main() {
  ...
  init("snake1", &snake, tail, START_TAIL_SIZE); //Инициализация, хвост = 3
  init("immortalSnake", &snake2, tail2,
       START_TAIL_SIZE); //Инициализация, хвост = 3
  ...
  while (key_pressed != STOP_GAME) {
    ...
    if (checkDirection(snake.direction,
                       key_pressed)) //Проверка корректности смены направления
    {
      changeDirection(&snake.direction,
                      key_pressed); // Меняем напарвление движения
    }
