...
/*
Add beep when snake eat food, add game over screen; add two game modes: with or without boarder of screen
*/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
 ...
 */
void go(struct snake *head, int boarder) {
    char ch[]="@";
    int max_x=0, max_y=0;
...
void printGameOver(struct snake *head)
{
	int max_x=0, max_y=0;
    getmaxyx(stdscr, max_y, max_x);
    
    printHelp("  Press 'q' for EXIT");
    mvprintw(max_y-6, max_x-45, "Your LEVEL is %d", head->tsize);
    
	mvprintw(max_y/2, max_x/2, "%s", "GAME");
	mvprintw(max_y/2+1, max_x/2, "%s", "OVER");
...
{
    char ch[]="*";
    int x=0, y=0, key_pressed=0, boarder=1;
    ...
    printHelp("  You can choose your game mode: boarder or without boarder. Press 'b' or 'b'");
    key_pressed = getch();
    if(key_pressed == 'b')
    {
        boarder = 1;
    ...
    while( key_pressed != STOP_GAME ) {
        ...
        go(&snake, boarder); // Рисуем новую голову
