...
enum {MAX_TAIL_SIZE=1000, MIN_TAIL_SIZE=2, START_TAIL_SIZE=3, MAX_FOOD_SIZE=20, MAX_BAD_FOOD_SIZE=3, FOOD_EXPIRE_SECONDS=10, SPEED=20000, SEED_NUMBER=3};
...
void initBadFood(struct bad_food bf[], size_t size) {
    struct bad_food init = {0,0,0,0,0};
    int max_y=0, max_x=0;
    getmaxyx(stdscr, max_y, max_x);
    for(size_t i=0; i<size; i++) {
        bf[i] = init;
...
void delTail(struct snake *head) {
    if(head == NULL || head->tsize<MIN_TAIL_SIZE) {
        mvprintw(0, 0, "Can't add tail");
        return;
    }
    head->tsize--;
...
void putBadFoodSeed(struct bad_food *fp) {
    int max_x=0, max_y=0;
    char spoint[2] = {0};
    getmaxyx(stdscr, max_y, max_x);
    mvprintw(fp->y, fp->x, " ");
...
void blinkBadFood(struct bad_food fp[], size_t nfood) {
    time_t current_time = time(NULL);
    char spoint[2] = {0}; // как выглядит зерно '@','\0'
    for( size_t i=0; i<nfood; i++ ) {
        if( fp[i].enable && (current_time - fp[i].put_time) > 6 ) {
            spoint[0] = (current_time % 2)? 'A' : 'a';
            mvprintw(fp[i].y, fp[i].x, spoint);
        }
    }
}
...
void repairBadSeed(struct bad_food bf[], size_t nfood, struct snake *head) {
    for( size_t i=0; i<head->tsize; i++ )
        for( size_t j=0; j<nfood; j++ ){
            /* Если хвост совпадает с зерном */
            if( bf[j].x == head->tail[i].x && bf[j].y == head->tail[i].y && bf[i].enable ) {
                mvprintw(0, 0, "Repair tail seed %d",j);
                putBadFoodSeed(&bf[j]);
            }
        }
    for( size_t i=0; i<nfood; i++ )
        for( size_t j=0; j<nfood; j++ ){
            /* Если два зерна на одной точке */
            if( i!=j && bf[i].enable && bf[j].enable && bf[j].x == bf[i].x && bf[j].y == bf[i].y && bf[i].enable ) {
                mvprintw(0, 0, "Repair same seed %d",j);
                putBadFoodSeed(&bf[j]);
            }
        }
...
void putBadFood(struct bad_food bf[], size_t number_seeds) {
    for(size_t i=0; i<number_seeds; i++) {
        putBadFoodSeed(&bf[i]);
    }
}
...
void refreshBadFood(struct bad_food bf[], int nfood) {
    int max_x=0, max_y=0;
    char spoint[2] = {0};
    getmaxyx(stdscr, max_y, max_x);
    for(size_t i=0; i<nfood; i++) {
        if( bf[i].put_time ) {
            if( !bf[i].enable || (time(NULL) - bf[i].put_time) > FOOD_EXPIRE_SECONDS ) {
                putBadFoodSeed(&bf[i]);
            }
        }
    }
...
_Bool haveEatBadFood(struct snake *head, struct bad_food bf[]) {
    for(size_t i=0; i<MAX_BAD_FOOD_SIZE; i++)
        if( bf[i].enable && head->x == bf[i].x && head->y == bf[i].y  ) {
            bf[i].enable = 0;
            return 1;
        }
    return 0;
...
int main()
{
    ...
    initBadFood(bad_food, MAX_BAD_FOOD_SIZE);
    ...
    putBadFood(bad_food, SEED_NUMBER);// Кладем зерна
    ...
    while( key_pressed != STOP_GAME ) {
        ...
        if(haveEatBadFood(&snake, bad_food)) {
            delTail(&snake);
//            printLevel(&snake);
        }
        refreshFood(food, SEED_NUMBER);// Обновляем еду
        refreshBadFood(bad_food, SEED_NUMBER);
        repairSeed(food, SEED_NUMBER, &snake);
        repairBadSeed(bad_food, SEED_NUMBER, &snake);
        blinkFood(food, SEED_NUMBER);
        blinkBadFood(bad_food, SEED_NUMBER);
