...
void handleKeyPress(struct snake *head, int32_t key) {
    switch (key) {
...
void printLevelInfo(struct snake *head) {
    int max_x=0, max_y=0;
    getmaxyx(stdscr, max_y, max_x);
    mvprintw(0, max_x-10, "LEVEL: %ld", head->tsize);
    mvprintw(0, max_x-20, "SPEED: %d", head->speed);
}
...
void increaseSpeed(struct snake *head) {
    head->speed++;
}
...
int main()
{
    ...
    printLevelInfo(&snake);
    while( key_pressed != STOP_GAME ) {
        key_pressed = getch(); // Считываем клавишу
        handleKeyPress(&snake, key_pressed); // Меняем напарвление движения
        if(isCrash(&snake))
            break;
